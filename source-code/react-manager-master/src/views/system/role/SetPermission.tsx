...
export default function SetPermission(props: IModalProp<Role.RoleItem>) {
  ...
  const onCheck: TreeProps<Menu.MenuItem>['onCheck'] = (checkedKeysValue, item) => {
    const nextCheckedKeys = Array.isArray(checkedKeysValue) ? checkedKeysValue : checkedKeysValue.checked
    setCheckedKeys(nextCheckedKeys.map(String))
    ...
    const halfCheckedKeys = (item.halfCheckedKeys || []).map(String)
    setPermission({
      _id: roleInfo?._id || '',
      permissionList: {
        checkedKeys,
